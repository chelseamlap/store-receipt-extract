...
> **HAR addendum (2026-05-22):** in a fresh capture the `order_history` call to
> `api.target.com` sent this same key as an **`x-api-key` request header**
> (not only as a query param). A second public key `5622b71f...` appeared on
> other surfaces. The implementation sends `x-api-key: <key>` on the
> `api.target.com` order calls alongside `credentials: 'include'`.
